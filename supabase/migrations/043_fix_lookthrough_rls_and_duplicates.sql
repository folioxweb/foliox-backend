-- ============================================================================
-- 043_fix_lookthrough_rls_and_duplicates.sql
-- Fixes:
-- 1. Drops and recreates get_stock_lookthrough with SECURITY INVOKER so RLS
--    (auth.uid() = user_id) is strictly enforced, preventing multi-user leak.
...
-- 2. Drop existing function to ensure signature & security attributes change cleanly
DROP FUNCTION IF EXISTS public.get_stock_lookthrough(text);

-- 3. Create get_stock_lookthrough with SECURITY INVOKER
