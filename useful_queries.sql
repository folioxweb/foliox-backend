-- ============================================================================
-- FOLIOX / EQUITY DASHBOARD - ESSENTIAL SQL QUERIES REFERENCE
-- ============================================================================
-- Clean, simple queries to inspect, verify, and debug data across all features.
...
-- 1.2 User activity summary (count of holdings, watchlist items, and virtual trades)
SELECT 
    u.email,
    COUNT(DISTINCT t.asset_id) AS total_holdings,
    COUNT(DISTINCT w.symbol) AS watchlist_items,
    COUNT(DISTINCT pt.id) AS paper_trades
FROM auth.users u
LEFT JOIN transactions t ON t.user_id = u.id
LEFT JOIN watchlist_items w ON w.user_id = u.id
LEFT JOIN paper_transactions pt ON pt.user_id = u.id
...
SELECT 
    ...
    ROUND(invested_value, 2) AS invested_value, 
    ROUND(current_value, 2) AS current_value, 
    ROUND(return_abs, 2) AS return_abs, 
...
SELECT 
    ...
    ROUND(return_pct, 2) AS return_pct, 
    ROUND(day_change, 2) AS today_pnl 
FROM vw_portfolio_summary;

-- 2.6 Sector-wise allocation
SELECT 
    sector, 
    ROUND(allocated_value, 2) AS total_value, 
    ROUND(allocation_pct, 2) AS percentage 
...
SELECT 
    ...
    ROUND(t.quantity * t.price, 2) AS total_amount,
    t.user_id
FROM transactions t
...
FROM transactions t
JOIN assets a ON a.asset_id = t.asset_id
WHERE a.symbol = 'TCS' OR a.symbol = 'NSE:TCS'
ORDER BY t.tx_date DESC;
...
SELECT 
    ...
    a.symbol, 
    sip.sip_amount, 
    sip.sip_day AS day_of_month, 
    sip.is_enabled, 
    sip.last_sip_date 
FROM mf_sip_configs sip
JOIN assets a ON a.asset_id = sip.asset_id
ORDER BY sip.is_enabled DESC, sip.sip_day ASC;
...
SELECT 
    ...
    last_updated 
FROM mf_schemes 
WHERE name ILIKE '%Parag Parikh%' OR name ILIKE '%Nifty 50%'
ORDER BY name ASC 
LIMIT 10;

-- 4.3 View top underlying stock holdings inside a Mutual Fund
SELECT 
    a.name AS mutual_fund, 
    fh.company_name AS holding_stock, 
    fh.symbol, 
    fh.sector, 
    fh.allocation_pct 
FROM fund_holdings fh
JOIN assets a ON a.asset_id = fh.fund_asset_id
ORDER BY fh.allocation_pct DESC 
LIMIT 20;
...
SELECT 
    stock_symbol, 
    stock_name, 
    ROUND(indirect_invested_value, 2) AS indirect_value 
FROM vw_indirect_exposure 
ORDER BY indirect_invested_value DESC 
LIMIT 10;
...
SELECT 
    ...
    confidence, 
    badge, 
    ROUND(added_price, 2) AS added_price, 
    ROUND(current_price, 2) AS live_price, 
    ROUND(target_price, 2) AS target_price, 
    ROUND(return_since_added_pct, 2) AS return_pct,
    added_at 
FROM vw_watchlist 
...
SELECT 
    ...
    current_price, 
    target_price, 
    ROUND(((target_price - current_price) / current_price) * 100, 2) AS upside_potential_pct 
FROM vw_watchlist 
WHERE target_price > current_price 
ORDER BY upside_potential_pct ASC;
...
SELECT 
    ...
    ROUND(current_cash, 2) AS cash_balance, 
...
SELECT 
    ...
    ROUND(invested_value, 2) AS invested_value, 
    ROUND(current_value, 2) AS current_value, 
    ROUND(return_pct, 2) AS profit_loss_pct 
...
SELECT 
    ...
    pt.tx_type, 
    pt.quantity, 
    ROUND(pt.price, 2) AS trade_price, 
    ROUND(pt.realized_gain, 2) AS realized_gain 
FROM paper_transactions pt
JOIN paper_assets pa ON pa.asset_id = pt.asset_id
...
SELECT 
    ...
    listing_date, 
    gmp_amount, 
    ROUND(gmp_percent, 2) AS gmp_percent 
FROM mainboard_ipos 
...
SELECT 
    ...
    updated_on_text 
FROM mainboard_ipos 
WHERE gmp_percent IS NOT NULL 
ORDER BY gmp_percent DESC 
LIMIT 10;
...
SELECT 
    ...
    h.gmp, 
    m.ipo_name 
FROM ipo_gmp_history h
JOIN mainboard_ipos m ON m.id = h.ipo_id
WHERE m.ipo_name ILIKE '%Tata%' OR m.id = 1
...
SELECT 
    ipo_name, 
    alert_type, 
    ROUND(gmp_percent, 2) AS gmp_pct, 
    recipient_count, 
    sent_status, 
    error_message, 
    created_at 
...
SELECT 
    ...
    title, 
    ai_model, 
    ai_summary_json ->> 'sentiment' AS sentiment, 
    ai_summary_json ->> 'summary' AS executive_summary,
    created_at 
FROM company_documents 
...
SELECT 
    ...
    last_updated 
FROM nse_stocks 
WHERE symbol ILIKE 'HDFC%' OR name ILIKE '%Reliance%'
ORDER BY symbol ASC 
LIMIT 10;
...
SELECT 
    ...
    created_at 
FROM system_execution_logs 
WHERE status = 'FAILED' OR response_status >= 400 
ORDER BY id DESC 
LIMIT 15;
...
SELECT 
    ...
    response_data 
FROM system_execution_logs 
WHERE function_name = 'sync-prices' 
ORDER BY id DESC 
LIMIT 10;
...
SELECT 
    ...
    created_at 
FROM user_audit_logs 
WHERE user_email = 'parthdeshmukh291@gmail.com' 
ORDER BY id DESC 
LIMIT 20;
